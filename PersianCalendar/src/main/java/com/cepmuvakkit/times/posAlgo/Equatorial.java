...
class Equatorial {
    double α; //right ascension (α) -also RA-, or hour angle (H) -also HA-
    double δ; //declination (δ)
    double Δ; //distance to the earth(Δ) in km
    ...
    Horizontal Equ2Topocentric(double longitude, double latitude, double Height, double jd, double ΔT) {
    ...
    private double ρsinϕPrime(double ϕ, double Height) {
        double U = MATH.atan(0.99664719 * Math.tan(ϕ));
        return 0.99664719 * Math.sin(U) + (Height / 6378149 * Math.sin(ϕ));
    }

    private double ρCosϕPrime(double ϕ, double Height) {
